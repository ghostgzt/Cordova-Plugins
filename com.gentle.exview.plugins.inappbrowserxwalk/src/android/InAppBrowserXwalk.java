...
import com.gentle.exview.plugins.BrowserDialog;
...
public class InAppBrowserXwalk extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("injectScriptCode")) {
            ...
            if (args.getBoolean(1)) {
                jsWrapper = String.format("(function(){eval(%%s)})()", callbackContext.getCallbackId());
            }
       ...
       if (action.equals("injectScriptFile")) {
            String jsWrapper;
            if (args.getBoolean(1)) {
                jsWrapper = String.format("(function(d) { var c = d.createElement('script'); c.src = %%s; c.onload = function() {  }; d.body.appendChild(c); })(document)", callbackContext.getCallbackId());
            } else {
        ...
        if (action.equals("injectStyleCode")) {
            String jsWrapper;
            if (args.getBoolean(1)) {
                jsWrapper = String.format("(function(d) { var c = d.createElement('style'); c.innerHTML = %%s; d.body.appendChild(c);})(document)", callbackContext.getCallbackId());
            } else {
        ...
        if (action.equals("injectStyleFile")) {
            String jsWrapper;
            if (args.getBoolean(1)) {
                jsWrapper = String.format("(function(d) { var c = d.createElement('link'); c.rel='stylesheet'; c.type='text/css'; c.href = %%s; d.head.appendChild(c); })(document)", callbackContext.getCallbackId());
            } else {
    ...
    class MyResourceClient extends XWalkResourceClient {
           ...
           @Override
           public void onReceivedLoadError (XWalkView view, int paramInt, String msg, String url) {
               try {
                   JSONObject obj = new JSONObject();
                   obj.put("type", "loaderror");
                   obj.put("code", paramInt);
				   obj.put("url", url);
				   obj.put("msg", msg);
                   PluginResult result = new PluginResult(PluginResult.Status.OK, obj);
                   result.setKeepCallback(true);
